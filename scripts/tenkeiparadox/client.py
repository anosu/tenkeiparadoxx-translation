...
class TenkeiparadoxClient:
    ...
    def request(self, method: Literal['GET', 'POST'], path: str, payload=None, msgpack: bool = True):
        ...
        if msgpack:
            setattr(resp, '_content', json.dumps(deserialize_api(resp.content), ensure_ascii=False).encode())
        if not self.version:
